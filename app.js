...
const CATEGORY_KEYWORDS_LOWER = Object.fromEntries(
    Object.entries(CATEGORY_KEYWORDS).map(([k, arr]) => [k, arr.map(w => w.toLowerCase())])
);
...
function loadTodos() {
    if (todosCache !== null) return todosCache;
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return (todosCache = []);
    try {
        const parsed = JSON.parse(raw);
        todosCache = Array.isArray(parsed) ? parsed.filter(t => t && typeof t.id === "string" && typeof t.text === "string") : [];
        return todosCache;
    } catch {
        console.warn("localStorage의 할 일 데이터가 손상되어 초기화합니다.");
        return (todosCache = []);
    }
}
...
function saveTodos(todos) {
    ...
    try {
        ...
        console.warn("할 일을 저장하지 못했습니다.", e);
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    todoInputEl.addEventListener("keydown", (e) => {
        if (e.key === "Enter" && !e.isComposing && e.keyCode !== 229) handleAdd();
    });
    todoInputEl.addEventListener("input", debounce(updateAutoHint, 150));
